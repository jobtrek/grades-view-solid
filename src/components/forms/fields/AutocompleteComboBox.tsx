...
export const AutocompleteComboBox: Component = (props) => {
  return (
    <Combobox.Root
      ...
      class="sm:col-span-4"
    >
      <Combobox.Label class="block text-sm font-medium leading-6 text-gray-900">Framework</Combobox.Label>
              ...
              <For each={epsicModules}>
                {(item) => (
                  <Combobox.Item
                    ...
                    item={item}>
                    <Combobox.ItemText>
                      <span class="block truncate">{item.no} - {item.description}</span>
                    </Combobox.ItemText>
                    <Combobox.ItemIndicator asChild={true}>
                      <span
                        class="absolute inset-y-0 left-0 items-center pl-1.5 hidden text-indigo-600 data-[state=checked]:flex data-[state=checked]:text-white">
